...
const useClosestSite = (sites) => {
    ...
    const onFailedToGetUserLocation = (e) => {
        ...
        setState('error')
        if (e.code === 1) {
            setErrMsg("Konum verisine ulaşımı engellediğiniz için isteğinizi yerine getiremiyoruz. İsterseniz konum verilerine erişimi arama barının sol kısmında kalan konum simgesine tıklayarak açabilirsiniz.")
        }
        else if (e.code === 2) {
