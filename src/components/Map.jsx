...
const Map = ({
  ...
  const getPinForSite = (siteType) => {
    return siteType === "SHELTER"
      ? new L.icon({ iconSize: [35], iconUrl: require("./img/house.png") })
  ...
  return (
    ...
    >
      ...
      {sites
        .filter(
          (site) =>
            site.location && site.location.latitude && site.location.longitude
        )
        .map((site) => {
          return (
            <Marker
      ...
      <AddingSiteComponent></AddingSiteComponent>
    </MapContainer>
  );
};
